...
class PersonalizedCourseCreationViewModel @Inject constructor(
    ...
    private val _courseGenerationResult = MutableLiveData<Result<ApiResult<CourseGenerationResponse>>>()
    val courseGenerationResult: LiveData<Result<ApiResult<CourseGenerationResponse>>> = _courseGenerationResult
    ...
    private fun generateCourse(userId: String, prompt: String, language: String) {
        viewModelScope.launch {
            val result = try {
                Result.success(personalizedTextCourseRepository.generateCourse(userId, prompt, language))
            } catch (e: Exception) {
                Result.failure(e)
            }
